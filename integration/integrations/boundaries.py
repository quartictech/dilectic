...
def postcode_districts_table(cfg):
    return db_create(cfg, 'postcode_districts', sql_file=os.path.join(cfg.derived_dir, 'postcode_districts.sql'), db_config=cfg.config["db"])

@task
def lsoa_2001_table(cfg):
    return db_create(cfg, 'lsoa_2001_ew_bfe_v2', sql_file=os.path.join(cfg.derived_dir,'lsoa_2001_ew_bfe_v2.sql'), db_config=cfg.config["db"])

@task
def lsoa_2011_table(cfg):
    return db_create(cfg, 'lsoa_2011_ew_bfe_v2', sql_file=os.path.join(cfg.derived_dir,'lsoa_2011_ew_bfe_v2.sql'), db_config=cfg.config["db"])

@task
def lsoa_2011_london_gen_mhw_table(cfg):
    return db_create(cfg, 'lsoa_2011_london_gen_mhw', sql_file=os.path.join(cfg.derived_dir,'lsoa_2011_london_gen_mhw.sql'), db_config=cfg.config["db"])

@task
def london_borough_excluding_mhw_table(cfg):
    return db_create(cfg, 'london_borough_excluding_mhw', sql_file=os.path.join(cfg.derived_dir, 'london_borough_excluding_mhw.sql'), db_config=cfg.config["db"])

@task
def local_authority_green_belt_boundaries_2014_15(cfg):
    return db_create(cfg, 'local_authority_green_belt_boundaries_2014_15', sql_file=os.path.join(cfg.derived_dir, 'local_authority_green_belt_boundaries_2014_15.sql'), db_config=cfg.config["db"])

@task
def postcode_districts_clean(cfg):
    return db_create(cfg, 'postcode_districts_clean',
...
def lsoa_2001_clean(cfg):
    return db_create(cfg, 'lsoa_2001_ew_bfe_v2_clean',
...
def lsoa_2011_clean(cfg):
    return db_create(cfg, 'lsoa_2011_ew_bfe_v2_clean',
